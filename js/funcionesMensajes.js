...
function putMensajes(idBotonActualizar){
    ...
    $.ajax({
        ...
        data: JSON.stringify(cajas),
        success:function(respuesta){
            alert("se ha actualizodo con exito");
            window.location.reload();
        }
    });
}
...
function pintarRespuesta(items){
    ...
    for(i=0;i<items.length;i++){
        ...
        myTable+="<td> <button onclick='putMensajes("+items[i].idMessage+")'class='flex mx-auto text-white bg-cyan-800 border-0 py-2 px-8 focus:outline-none hover:bg-cyan-900 rounded text-lg'> Actualizar </button> " ;
        myTable+="<td> <button onclick='deleteMensajes("+items[i].idMessage+")'class='flex mx-auto text-white bg-cyan-800 border-0 py-2 px-8 focus:outline-none hover:bg-cyan-900 rounded text-lg'> Borrar </button> " ;
        myTable+="</tr>";
    }
    myTable+="</table>";
