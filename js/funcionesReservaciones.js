...
function postReservaciones(){
    ...
    || $("#select-car").val().length==0 || $("#select-client").val().length==0){
    ...
    $.ajax({
        url:"http://152.70.120.190:8080/api/Reservation/save",
        type:"POST",
        datatype:"JSON",
...
function putReservaciones(idBotonActualizar){
    ...
    $.ajax({
        url:"http://152.70.120.190:8080/api/Reservation/update",
        type:"PUT",
        datatype:"JSON",
        contentType: "application/json; charset=utf-8",
        data: JSON.stringify(cajas),
...
function deleteReservaciones(idBoton){
    Swal.fire({
        title: 'Esta seguro de borrar la reservacion? con el id:'+idBoton,
    ...
    $.ajax({
        url:"http://152.70.120.190:8080/api/Reservation/"+idBoton,
        type:"DELETE",
        datatype:"JSON",
...
function pintarRespuesta(items){
    ...
    for(i=0;i<items.length;i++){
        ...
        myTable+="<td> <button onclick='putReservaciones("+items[i].idReservation+")'class='flex mx-auto text-white bg-cyan-800 border-0 py-2 px-8 focus:outline-none hover:bg-cyan-900 rounded text-lg'> Actualizar </button> " ;
        myTable+="<td> <button onclick='deleteReservaciones("+items[i].idReservation+")'class='flex mx-auto text-white bg-cyan-800 border-0 py-2 px-8 focus:outline-none hover:bg-cyan-900 rounded text-lg'> Borrar </button> " ;
        myTable+="</tr>";
    }
    myTable+="</table>";
